...
7. Avoid `this.` unless you need to differentiate current type members from other symbols.

8. Explicitly write visibility for modules even if they have implicit default visibilities.

9. Use `var` only if the type of the variable can be determined reasonably from the immediate context.

10. Put `readonly` after `static` static.
